...
class Suggest:
    ...
    def parse_suggestions(self):
        ...
        for _, title, pitch, runtime, year, notes in self.file_contents[1:]:
            ...
            pitch = pitch.replace("\n+", "\n").strip()
            self.parsed_suggestions.append(
                [title, f"{title} ({year}, {h}h{m:0>2}m)", f"{pitch}{notes}"]
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse suggest movies into strings for usage in runoff ballots"
    )
    parser.add_argument(
        "-s",
        "--select",
    ...
    parser.add_argument(
        "-o",
        "--outfile",
        help="save parsed ballots to a file instead of terminal",
        action="store_true",
    )
